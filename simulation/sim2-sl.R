...
noise_sl <- 0.057 * c(0.2, 1, 5) # sd(beta X):sd(eplsilon) = 5, 1, 0.2



# sparse tree: Y = 3 * SylFrontSul_L - 2 * SylParieSul_L - SylTempSul_L
beta_true <- c(rep(0,5), 3, -2, -1, rep(0, 358))
...
for(t in 1:length(sim2_sl)){
  sim2_sl[[t]] <- foreach(j = 1:n_sim, .combine = cbind, .packages = packages) %dopar% {
    ...
    Y <- 3 * X.leaf[,6] - 2 * X.leaf[,7] - X.leaf[,8] + rnorm(n, sd = noise_sl[t])
