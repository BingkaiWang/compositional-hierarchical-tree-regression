...
noise_st <- sd(3 * rowSums(X_complete[,which(sim.tree$level1 == "CSF.lvl1")]) - 
                 2 * rowSums(X_complete[,which(sim.tree$level1 == "Mesencephalon.lvl1")]) - 
...
beta_true <- c(rep(0,365), -2, -1, 0, 3, 0)
sim2_sl <- vector("list", length(noise_st))
for(t in 1:length(sim2_sl)){
  sim2_sl[[t]] <- foreach(j = 1:n_sim, .combine = cbind, .packages = packages) %dopar% {
    ...
    Y <- 3 + 3 * rowSums(X.leaf.centered[,which(sim.tree$level1 == "CSF.lvl1")]) - 
      2 * rowSums(X.leaf.centered[,which(sim.tree$level1 == "Mesencephalon.lvl1")]) - 
