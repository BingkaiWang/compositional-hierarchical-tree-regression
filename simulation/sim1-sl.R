...
param_grid <- expand.grid(eta = seq(0.0, 1, by = 0.05), gamma = c(11e-4, 1e-2))
...
for(t in 1:length(sim1_sl)){
  sim1_sl[[t]] <- foreach(j = 1:n_sim, .combine = cbind, .packages = packages) %dopar% {
    ...
    Y <- 2 * X.leaf[,1] - X.leaf[,3] - X.leaf[,4] + rnorm(n, sd = noise_sl[t])
    ...
    result[11:14] <- tryCatch(
      {
        ...
        alpha <- coef(sim.fit, lambda = sim.fit$lambda[which.min(IC)])$beta
        beta <- calculate_beta(sim.tree, alpha)
        c(sensitivity = mean(abs(beta[which(abs(beta_true)>0)]) > 0.01), 
          specifity = mean(abs(beta[which(abs(beta_true)==0)]) < 0.01), MSE = sum((beta - beta_true)^2), ENP = sum(abs(beta) > 0.01))
      },
      error = function(err) {print(err); rep(NA,4)}
    )
    
    # TASSO with BIC
    result[15:18] <- tryCatch(
      {
        tunning_param <- map_dbl(1:nrow(param_grid_tasso), function(j){
          sim.fit <- genlasso(y = Y, X = X.leaf, D = construct_TASSO(sim.tree), eps = param_grid_tasso[j,1])
          df <- sim.fit$df - n
